...
기준 문서는 **FE 웹 수정요청서 2026-09-20** 이며, 2026-09-17 전달본을 대체한다.
요약하면 이렇다.

- QR은 팔찌·환자 앱이 공유하는 반영구 링크 하나다: `/emergency#card=<opaque-reference>`.
- 웹은 `card` 참조값을 **읽지 않고**, 모든 진입에 같은 119 신고 안내를 보여준다.
- 공개 환자조회 API(`card-sessions`, `emergency-access`, 과거 `report-complete`)는 **폐지**됐다.
  410 `PUBLIC_DISCLOSURE_RETIRED`는 기능 폐지를 뜻하며 "환자가 카드를 철회함"이 아니다.
- 백엔드가 호출 가능한 경로는 `POST /api/public/v1/location/reverse-geocode` 하나뿐이다.
- 위치 권한 거절·시간 초과·503에서도 119 안내는 계속 동작해야 한다.

구현·검증 상태는 [README](../README.md) 참조. 아래 내용은 **2026-09-16 시점의 이력**이며,
현재 사양과 어긋나는 부분이 있다(특히 Netlify Function 역지오코딩 — 백엔드가 같은 계약을
제공하면서 2026-09-20에 제거했다).
